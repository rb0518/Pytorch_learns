...
// -----------------------------------
Loss::Loss(const int64_t class_num_, const int64_t ng_, const int64_t nb_) {
...
std::tuple<torch::Tensor, torch::Tensor> Loss::build_target(std::vector<std::tuple<torch::Tensor, torch::Tensor>>& target) {
	...
	target_class = torch::full({ (int64_t)mini_batch_size, ng, ng, class_num }, /*value=*/0.0, torch::TensorOptions().dtype(torch::kFloat));  // target_class{N,G,G,CN}
	target_coord = torch::full({ (int64_t)mini_batch_size, ng, ng, 5 }, /*value=*/0.0, torch::TensorOptions().dtype(torch::kFloat));  // target_coord{N,G,G,5}
	...
	for (size_t n = 0; n < mini_batch_size; n++) 
	{
		...
		for (size_t b = 0; b < BB_n; b++) 
		{
			...
			i = (size_t)(fabs(cx / cell_size));  // i[0,G)
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> Loss::operator()(torch::Tensor& input, std::vector<std::tuple<torch::Tensor, torch::Tensor>>& target) {
	...
	int64_t mini_batch_size = input.size(0);
