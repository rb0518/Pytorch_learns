...
class VOCDataset(Dataset):
    def __init__(self, is_train, image_dir, label_txt, image_size = 448, grid_size = 7, num_bboxes = 2, num_classes = 20):
        super().__init__()
        self.is_train = is_train            # 数据类型是否是train
        self.image_size = image_size        # 448
        self.grid_size = grid_size          # grid is 7 * 7
        self.num_bboxe = num_bboxes         # 2
        self.num_classes = num_classes      # 20

        mean_rgb = [122.67891434, 116.66876762, 104.00698793]   # 网上举例都是这个数
        ...
        for line in lines:
            ...
            num_bboxes = (len(splitted) -1) // 5
            box, label = [], []
            for i in range(num_bboxes):
                x1 = float(splitted[1 + 5*i])
                y1 = float(splitted[2 + 5*i])
                x2 = float(splitted[3 + 5*i])
                y2 = float(splitted[4 + 5*i])
                cid = int(splitted[ 5 + 5 * i])
    ...
    def __getitem__(self, index):
        ...
        for b in range(n):
            ...
            cv2.rectangle(img_show, pt1 = pt1, pt2 = pt2, color = (20, 20, 220), thickness=1)
        cv2.imwrite(os.path.join(debug_dir, 'test_{}.jpg'.format(index)), img_show)

        h, w, _ = img.shape
        boxes /= torch.Tensor([w,h,w,h]).expand_as(boxes)
        target = self.encode(boxes, labels)  # [7(grid_size), 7, 5 * 2(num_bbox) + 20(num_class)]
    ...
    def encode(self, boxes, labels):
        ...
        S, B, C = self.grid_size , self.num_bboxe, self.num_classes
        N = 5 * B + C
