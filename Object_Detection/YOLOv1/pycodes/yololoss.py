...
class YoloLoss(nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        if self.use_gpu:
            noobj_target_mask = torch.cuda.BoolTensor(noobj_target.size())
        else:
            noobj_target_mask = torch.BoolTensor(noobj_target.size())
        ...
        if self.use_gpu:
            coord_response_mask = torch.cuda.BoolTensor(box_target.size())
            coord_not_response_mask = torch.cuda.BoolTensor(box_target.size())
        else:
            coord_response_mask = torch.BoolTensor(box_target.size())
            coord_not_response_mask = torch.BoolTensor(box_target.size())
