...
class VGGNet(VGG):
    def __init__(self, pretrained=True, model='vgg16', requires_grad=True, remove_fc=True, show_params=False, dataroot='.'):
        super().__init__(make_layers(cfg[model]))
        self.ranges = ranges[model]

        if pretrained:
            vgg16 = models.vgg16(pretrained=False)
            filename = os.path.join(dataroot, 'vgg16-397923af.pth')
            vgg16.load_state_dict(torch.load(filename))
