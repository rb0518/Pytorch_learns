...
#include "util.h"
Conv3x3GNReLUImpl::Conv3x3GNReLUImpl(int in_channels, int out_channels, bool upsample /* = false */)
{
	this->upsample = upsample;
	block = torch::nn::Sequential(
		torch::nn::Conv2d(conv_options(in_channels, out_channels, 3, 1, 1, 1, false)),
		torch::nn::GroupNorm(torch::nn::GroupNormOptions(32, out_channels)),
...
torch::Tensor Conv3x3GNReLUImpl::forward(torch::Tensor x)
{
	...
	if (upsample)
	{
		x = torch::nn::Upsample(upsample_options(std::vector<double>{2.0, 2.0}))->forward(x);
	}

	return x;
}

FPNBlockImpl::FPNBlockImpl(int pyrmid_channels, int skip_channels)
{
	skip_conv = torch::nn::Conv2d(conv_options(skip_channels, pyrmid_channels, 1));
	upsample = torch::nn::Upsample(torch::nn::UpsampleOptions().scale_factor(std::vector<double>({ 2.0, 2.0 })).mode(torch::kNearest));
...
{
	CHECK(policy == policies[0] || policy == policies[1]) << "merge_policy muse be one of 'add'  or 'cat',  " << policy;
	_policy = policy;
}

torch::Tensor MergeBlockImpl::forward(std::vector<torch::Tensor> x)
{
	CHECK(x.size() > 0) << "input list size is empty";
	torch::Tensor re;
	if (_policy == "add")
	{
		re = x[0];
	...
	float dropout_, std::string merge_policy)
{
	...
	if (encoder_depth < 3) 
		std::cout << "Encoder depth for FPN decoder cannot be less than 3";
