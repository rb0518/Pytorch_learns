...
VGGImpl::VGGImpl(std::vector<int> cfg, int num_classes /* = 1000 */, bool batch_norm /* = false */) 
{
	this->cfg = cfg;
	this->batch_norm = batch_norm;
...
void VGGImpl::make_dilated(std::vector<int> stage_list, std::vector<int> dilation_list) {
	LOG(WARNING) << "'VGG' models do not support dilated mode due to Max Pooling operations for downsampling!";
	return;
}
