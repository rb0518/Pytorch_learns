...
#else
	auto vect = for_each_file(folder, std::string(".json"), false);
	for_each(vect.begin(), vect.end(), [&](std::string s) {
		list_labels.push_back(s);
		std::string image_path = std::filesystem::path(s).replace_extension(image_type).string();
		list_images.push_back(image_path);
		});
#endif
}
